            ...
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList"
                ...
                <dd changepirce="0"
                    ...
                    @click="changeActive(SaleAttrValue, spuSaleAttr.spuSaleAttrValueList)">
...
export default {
  ...
  methods: {
    ...
    // 表单元素修改产品个数
    changeSkuNum(event) {
      // 若用户输入文本 * 1，可判断为非数字(非法的) ---NaN
      let value = event.target.value * 1
      // 所以判断若收到非法的值或负值，就重置为1
    ...
    async addShopCart() {
      ...
      try {
        ...
        // 若成功，就路由跳转，并把参数带给下一级路由组件addcartsuccess
        // 产品信息的数据，需通过会话存储（会话结束，数据再消失）---把存储的对象信息转为字符串
