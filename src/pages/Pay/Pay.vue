    ...
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr"><em class="lead">应付金额：</em><em class="orange money">￥{{payInfo
          .totalFee}}</em></span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    // 获取支付的信息
    async getPayOrder() {
      let result = await this.$API.reqGetPayOrder(this.orderId)
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, '请您微信支付', {
        ...
        beforeClose: (type, instance, done) => {
          // type:区分取消/确定按钮   instance:当前的组件实例   done:关闭弹出框的方式
          if (type == 'cancel') {
            alert('请联系管理员')
            // 清除定时器
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          ...
          console.log(result)
          // 判断：若支付成功，清除定时器，保存支付成功返回的状态码，并路由跳转至支付成功页面
          if (result.code == 200) {
            clearInterval(this.timer)
            this.timer = null
            // 保存支付成功返回的状态码
